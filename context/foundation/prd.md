...
### Primary
- User configures a dumbbell (handle + plates × count) and sees the correct total weight in kg.

### Secondary
- User can configure other equipment types (barbell, kettlebell) using the same mechanism.

### Guardrails
- The weight calculation must always be arithmetically correct — an error here undermines the entire purpose of the application.
- The application must be responsive and usable on a mobile browser (user accesses it during training).
- One user's data must never be accessible to another user (account isolation).
...
- **When** they enter a configuration name, handle weight, plate weight, and number of plates
- **Then** they see the total dumbbell weight calculated as: handle + (plate × count)

#### Acceptance Criteria
- All four inputs (name, handle weight, plate weight, plate count) are required before a result is shown
...
- The weight calculation result appears without any perceptible delay — the result updates immediately as inputs change.
...
Login required (email + password or OAuth). Flat user model — every registered user sees only their own weights and statistics. No roles in MVP. Post-MVP: a premium subscription tier is planned; the current flat model supports this addition without structural changes.
...
(none — all required elements present; quality check accepted during /10x-shape)
